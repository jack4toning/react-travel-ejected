...
const transformMenuList = (
  ...
  return menuList.map((menu: any, index: number) => {
    if (menu.subMenu) {
      return {
        label: menu.title,
        key: `layer-${layerNum}-menu-${idx}${index}`,
        children: transformMenuList(menu.subMenu, idx + index, layerNum),
      };
    } else
      return {
...
export const sideMenuItemList = transformMenuList(sideMenuList);
